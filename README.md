...
* Шаблон инфраструктуры (compose-файл с GitLab CE + Runner, пример пайплайна и пошаговая инструкция со скриншотами) лежит в `lab2/` — см. `lab2/README.md`.
...
6. Описать конвейер в `.gitlab-ci.yml` минимум с тремя обязательными стейджами — `prepare-dataset`, `train`, `test`. Связать конвейер с системой контроля версий (push в локальный GitLab → автозапуск pipeline).
